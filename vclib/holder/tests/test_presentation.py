...
def mock_data_with_2_creds(mock_data_with_cred, over_18_field_selection):
                ...
                "fields": [
                    {
                        ...
                        "filter": {"type": "string", "const": "value"},
                    }
                ]
            },
        }
    ...
    over_18_field_selection.field_requests.append(
        FieldRequest(
            field={
                "path": ["$.property", "$.credentialSubject.property"],
                "filter": {"type": "string", "const": "value"},
            },
            input_descriptor_id="field",
            approved=True,
        )
    )
...
async def test3_more_than_one_pd_type(httpx_mock: HTTPXMock, mock_data):
    ...
    over_18_auth_req["presentation_definition_uri"] = (
        "https://example.com/presentationdefs/over_18"
    )
    httpx_mock.add_response(
        url="https://example.com/request/over_18", json=over_18_auth_req
    )
...
async def test10_working_presentation(
    ...
    httpx_mock.add_response(url="https://example.com/request/over_18", json=auth_req)

    await holder.get_auth_request("https://example.com/request/over_18", auth_header)

    httpx_mock.add_response(url="https://example.com/cb", json={"status": "OK"})

    assert holder.current_transaction == AuthorizationRequestObject(**auth_req)
    resp = await holder.present_selection(over_18_field_selection, auth_header)
...
async def test14_requested_multiple_fields_from_credential(
    ...
    input_descriptors[0]["constraints"]["fields"].append(
        {
            "path": ["$.is_over_21", "$.credentialSubject.is_over_21"],
...
async def test16_request_property_in_payload(
            ...
            "input_descriptors": [
                {
                    ...
                    "constraints": {
                        "fields": [
                            {
                                "path": ["$.credentialSubject.iss", "$.iss"],
                                "filter": {
                                    "type": "string",
    ...
    selection = FieldSelectionObject(
        field_requests=[
            {
                "approved": True,
                ...
                "field": {
                    "path": ["$.credentialSubject.iss", "$.iss"],
                    "filter": {"type": "string", "const": "servicensw.com.au"},
                },
            }
        ]
    )
    ...
    httpx_mock.add_response(
        url="https://example.com/request/payload_property", json=auth_req
    )
