...
from vclib.holder.demo.demo_agent import DemoWebIdentityOwner
...
def identity_owner():
    id_owner = DemoWebIdentityOwner(
        ...
        mock_uri=OWNER_URI,
        oauth_client_options={"authorization_details_types": ["openid_credential"]},
    )
...
def test_get_pending_credentials(identity_owner):
    pending: list[credentials.DeferredCredential] = (
        identity_owner.get_deferred_credentials()
    )
...
async def test_authorize_issuer_initiated(identity_owner):
    ...
    select = credentials.CredentialOfferObject.model_validate(
        {
