...
async def test_presentation_request_limit_disclosure():
    ...
    pd = PresentationDefinition(id='test_limit_disclosure_1', input_descriptors=[InputDescriptor(id='limit_disclosure_test', constraints=Constraint([Field(['$.credentialSubject.active'])], limit_disclosure='required'))], name='required_limit_disclosure')
...
async def test_presentation_request_two_fields_optional():
    ...
    pd = PresentationDefinition(id='name_age_presentation_1', name='Age and Name request', purpose='To be able to address the customer by name, and verify their age', input_descriptors=[InputDescriptor( id='name_age_presentation_definition', name='name_age_presentation_definition', constraints=Constraint(fields=[Field(path=['$.credentialSubject.birthDate', '$.credentialSubject.dob', '$.credentialSubject.dateOfBirth'], name='date of birth check', id='dob'), Field(path=['$.credentialSubject.givenName'], name='given name request', id='given_name', optional=True)]), purpose='To be able to address the customer by name, and verify their age')])
...
async def test_presentation_request_not_found():
    ...
    with pytest.raises(HTTPException):
        await service_provider.get_presentation_request('non_existent', 'example_client_id')

@pytest.mark.asyncio
async def test_presentation_request_filter():
    sp = ServiceProvider("test_bundle", "test_path")

    pd = PresentationDefinition(id='test_filter', input_descriptors=[InputDescriptor(id='credit_card_test', constraints=Constraint([Field(path=['$.type'], filter=Filter(type='string', pattern='creditCard'))]))], name='required_limit_disclosure')
...
def create_dummy_certificate(private_key, public_key):
    ...
    subject = issuer = x509.Name([x509.NameAttribute(NameOID.COUNTRY_NAME, u"US"), x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, u"California"), x509.NameAttribute(NameOID.LOCALITY_NAME, u"San Diego"), x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"My Org"), x509.NameAttribute(NameOID.COMMON_NAME, u"example.com")])
    cert = x509.CertificateBuilder().subject_name(subject).issuer_name(issuer).public_key(public_key).serial_number(x509.random_serial_number()).not_valid_before(datetime.datetime.now(datetime.timezone.utc)).not_valid_after(datetime.datetime.now(datetime.timezone.utc) + timedelta(days=1)).add_extension(x509.SubjectAlternativeName([x509.DNSName(u"localhost")]),critical=False).add_extension(x509.UnrecognizedExtension(did_oid, b"My DID: did:example:123456789abcdefghi"), critical=False).sign(private_key=private_key, algorithm=hashes.SHA256())
...
async def test_verify_certificate_valid(service_provider):
    ...
    assert sp.verify_certificate(cert_pem=cert_pem, nonce=nonce, timestamp=timestamp) == did
